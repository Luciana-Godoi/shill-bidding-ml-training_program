...
tbl01 <- d_shillB |>
  ...
  tab_header(
    title = "Descriptive analysis of the Shill Bidding dataset"
  )
...
ajusta_xgb <- function(recipe_obj, metodo){
  ...
  saveRDS(
    xgb_hips,
    paste0(
      "hiperparametros_Optimal threshold using TS_",
      nome,
      ".rds"
    )
  )
  ...
  ###############################################################################
  ## Optimal threshold in training set (TS)
  ###############################################################################
  
  pred_tr_prob <- predict(
    xgb_final,
    new_data = d_tr,
    type = "prob"
  )
  ...
  saveRDS(
    xgb_final,
    paste0("modelo_Optimal threshold using TS_", nome, ".rds")
  )
  ...
  saveRDS(
    preds,
    paste0("preds_Optimal threshold using TS_", nome, ".rds")
  )
  ...
  saveRDS(
    mets,
    paste0("metrics_Optimal threshold using TS_", nome, ".rds")
  )
  ...
  saveRDS(
    resumo,
    paste0("resumo_Optimal threshold using TS_", nome, ".rds")
  )
...
write.csv(
  tbl_comp,
  "comparacao_metodos_balanceamento_Optimal threshold using TS.csv",
  row.names = FALSE
)
...
saveRDS(
  tbl_comp,
  "comparacao_metodos_balanceamento_Optimal threshold using TS.rds"
)
...
tbl_comp_transp |>
  ...
  tab_header(
    title = "Comparison of balancing methods - XGBOOST (TRH - TS)"
  )
...
preds_adasyn_OC <- readRDS(
  "preds_Optimal threshold using TS_ADASYN.rds"
)

modelo_smotenc_OC <- readRDS(
  "modelo_Optimal threshold using TS_SMOTENC.rds"
)

hips_bsmote_OC <- readRDS(
  "hiperparametros_Optimal threshold using TS_BSMOTE.rds"
)
